...
dotnet sitecore login --cm https://cm.jss18demo.localhost/ --auth https://id.jss18demo.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Invoke-RestMethod "https://cm.jss18demo.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.jss18demo.localhost/sitecore/
Start-Process https://www.jss18demo.localhost/
